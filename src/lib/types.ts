...
/** Public search: only `available` is shown; owner can mark `sold_out` when filled */
export type ListingAvailability = 'available' | 'sold_out'
...
export interface Property {
  ...
  amenities: string[]
  /** Owner-selected nearby landmarks (school, metro, etc.) */
  ...
  /** Omitted on legacy rows; treat as `available` */
...
/** Branded names for pricing UI and navbar (by `plan_type`). */
export function planDisplayNameFromType(
  planType: SubscriptionPlan | string | null | undefined,
): string {
  const labels: Record<string, string> = {
    day: 'Lite Plan (2-Day)',
    weekly: 'Relax Plan (Weekly)',
    monthly: 'Freedom Plan (Monthly)',
  }
  if (!planType) return ''
  return labels[planType] ?? String(planType)
}

/** Relax (weekly) & Freedom (monthly) include priority support; Lite (day) does not. */
export function subscriptionIncludesPrioritySupport(
  subscription: Subscription | null | undefined,
): boolean {
  if (!subscription?.is_active) return false
  if (new Date(subscription.expires_at) <= new Date()) return false
  const t = subscription.plan_type
...
export function normalizeNearbyPlaces(raw: unknown): string[] {
  if (raw == null) return []
  ...
  if (typeof raw === 'string') {
    ...
    try {
      ...
      return normalizeNearbyPlaces(parsed)
    } catch {
      return [t]
    }
  }
  if (typeof raw === 'object' && raw !== null && !Array.isArray(raw)) {
    ...
    if (vals.length && vals.every((v) => typeof v === 'string')) {
      return vals.map((s) => (s as string).trim()).filter(Boolean)
    }
  }
  return []
}
