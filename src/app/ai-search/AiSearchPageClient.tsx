    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section className="relative overflow-hidden pt-28 pb-10 sm:pt-32 sm:pb-14">
          <div
            className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top,_#ffffff_0%,_var(--background)_50%,_#e8f0f0_100%)]"
            aria-hidden
          />
          <div className="relative mx-auto max-w-3xl px-4 text-center sm:px-6">
            {!loadingAccess && (
              <div className="mb-5 flex justify-center">
                {!user ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card/80 px-3 py-1 text-xs text-muted-foreground backdrop-blur">
                    <Lock className="h-3.5 w-3.5" />
                    Sign in for 1 free try
                  </span>
                ) : access?.is_subscriber ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card/80 px-3 py-1 text-xs text-foreground backdrop-blur">
                    <CheckCircle2 className="h-3.5 w-3.5 text-primary" />
                    Included in your plan
                  </span>
                ) : access?.trial_remaining === 1 ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-accent/40 bg-card/80 px-3 py-1 text-xs text-foreground backdrop-blur">
                    <Sparkles className="h-3.5 w-3.5 text-accent" />1 free try
                    available
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card/80 px-3 py-1 text-xs text-muted-foreground backdrop-blur">
            ...
            <motion.div
              ...
              className="rounded-[1.75rem] border border-border bg-card p-5 text-left shadow-[0_20px_60px_-28px_rgba(8,59,58,0.18)] sm:p-6"
            >
              <Textarea
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-4">
                {EXAMPLE_QUERIES.map((ex) => (
                  <button
                    key={ex}
                    type="button"
                    className="max-w-[min(100%,18rem)] truncate rounded-full border border-border bg-transparent px-3 py-1.5 text-left text-xs text-muted-foreground transition-colors hover:border-primary/30 hover:text-foreground"
                    onClick={() => setQuery(ex)}
                  >
            ...
            {criteria && hasSearched && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="mx-auto mt-5 inline-flex max-w-full items-center rounded-full border border-border bg-card px-5 py-2.5 text-sm text-muted-foreground shadow-sm"
              >
        ...
        <section className="pb-16 sm:pb-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="mb-8 flex flex-col gap-3 sm:mb-10 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <h2 className="text-2xl mt-3 font-semibold tracking-tight sm:text-3xl">
                  Curated for you
                </h2>
            ...
            {searching && (
              <div className="mb-6 flex items-center justify-center gap-3 rounded-[1.5rem] border border-border bg-card py-16 text-muted-foreground shadow-sm">
            ...
            {!searching && hasSearched && results.length === 0 && (
              <div className="mb-6 rounded-[1.5rem] border border-border bg-card px-6 py-12 text-center shadow-sm">
              ...
              {curatedMatches.map((item, index) => (
                ...
                >
                  <Link
                    ...
                    className="group block h-full overflow-hidden rounded-[1.35rem] border border-border bg-card shadow-[0_12px_40px_-24px_rgba(8,59,58,0.22)] transition-shadow hover:shadow-[0_18px_48px_-20px_rgba(8,59,58,0.28)]"
                  >
                    <div className="relative aspect-[4/3] overflow-hidden">
                      ...
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-primary/80 via-primary/35 to-transparent p-4 pt-16">
                        <div className="rounded-xl border border-white/25 bg-white/15 px-3 py-2.5 backdrop-blur-md">
                    ...
                    <div className="flex flex-col gap-3 p-5">
                      ...
                      <div className="rounded-xl bg-secondary px-3.5 py-3 text-sm leading-relaxed text-muted-foreground">
                        “{item.aiAnalysis}”
                      </div>
                    </div>
                  </Link>
        ...
        <section className="pb-16 sm:pb-20">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div className="relative overflow-hidden rounded-[2rem] bg-primary px-6 py-12 text-primary-foreground sm:px-10 sm:py-14 lg:px-14">
              <div className="relative z-10 grid items-center gap-10 lg:grid-cols-[1.2fr_0.8fr]">
                <div>
                  ...
                  <p className="mb-8 max-w-md text-sm leading-relaxed text-primary-foreground/70 sm:text-base">
                  ...
                  >
                    <Input
                      ...
                      className="h-12 rounded-full border-white/15 bg-white/10 text-primary-foreground placeholder:text-primary-foreground/45 focus-visible:border-accent/50 focus-visible:ring-accent/30"
                    />
                    <Button
                      type="submit"
                      className="h-12 shrink-0 rounded-full bg-accent px-6 text-accent-foreground hover:bg-accent/90"
                    >
                      Get early access
                    </Button>
                  </form>
                ...
                <div className="relative mx-auto hidden h-48 w-48 items-center justify-center lg:flex">
                  ...
                  <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-accent/20">
                    <Bell className="h-7 w-7 text-accent" strokeWidth={1.5} />
                  </div>
                </div>
              </div>
            </div>
