...
export async function PUT(
  ...
  try {
    ...
    const missingNearby =
      !!error &&
      (msg.includes("nearby_places") ||
        (msg.includes("schema cache") && propertyData.nearby_places !== undefined));

    if (missingNearby) {
      const { nearby_places: _np, ...updateWithoutNearby } = updateData;
      console.warn(
        "⚠️ nearby_places column missing; retrying update without it. Run: ALTER TABLE properties ADD COLUMN nearby_places JSONB DEFAULT '[]'::jsonb;"
      );
      const retry = await supabaseAdmin
        .from("properties")
        .update(updateWithoutNearby)
        .eq("id", id)
        .select()
        .single();
      data = retry.data;
    ...
    return NextResponse.json({
      ...
      ...(missingNearby
        ? {
            warning:
              "Database is missing the nearby_places column. Add it in Supabase SQL: ALTER TABLE properties ADD COLUMN IF NOT EXISTS nearby_places JSONB NOT NULL DEFAULT '[]'::jsonb;",
          }
        : {}),
    });
  } catch (error) {
