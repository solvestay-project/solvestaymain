...
import type { PropertyListingMode } from "@/components/property/listing-mode";

type PropertyPgListingWizardProps = {
...
const adminPgSchema = pgSchema.extend({
...
export function PropertyPgListingWizard({
  mode = "owner",
}: PropertyPgListingWizardProps) {
  ...
  const {
    ...
    resolver: zodResolver(isAdminMode ? adminPgSchema : pgSchema) as Resolver<PgFormValues>,
    mode: "onChange",
    defaultValues: {
      security_deposit: 0,
      owner_name: "",
  ...
  useEffect(() => {
    async function checkVerification() {
      ...
      if (isAdminMode) {
        ...
        router.push("/admin/properties/new/pg");
        return;
      } else {
    ...
    switch (currentStep) {
      ...
      case 2:
        return await trigger(
          isAdminMode
            ? ["title", "description", "price", "security_deposit", "owner_name", "owner_phone"]
            : ["title", "description", "price", "security_deposit"]
        );
      case 3:
  ...
  const onSubmit: SubmitHandler<PgFormValues> = async (data) => {
    ...
    try {
      ...
      toast.success(
        isAdminMode
          ? "PG added successfully and is now live."
          : "PG listed successfully! It will be reviewed before going live."
      );
        ...
        >
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2">
              {isAdminMode ? "Add PG / Hostel" : "List PG / Hostel"}
            </h1>
            <p className="text-muted-foreground">
              {isAdminMode
                ? "Create a PG listing on behalf of an owner. Include their name and phone number."
                : "Fill in the details for your PG or hostel listing."}
            </p>
          </div>
                  ...
                  {isAdminMode && (
                    <div className="grid sm:grid-cols-2 gap-4 p-4 rounded-xl border border-primary/20 bg-primary/5">
                      <div>
                        ...
                        {errors.owner_name && (
                          <p className="text-sm text-destructive mt-1">{errors.owner_name.message}</p>
                        )}
                      </div>
                      <div>
                        ...
                        {errors.owner_phone && (
                          <p className="text-sm text-destructive mt-1">{errors.owner_phone.message}</p>
                        )}
                      </div>
                    </div>
                  )}
                    ...
                    {isAdminMode && (
                      <div className="grid sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-medium">{watchedValues.owner_name}</p>
                        </div>
                        <div>
                          <span className="text-sm text-muted-foreground">Owner Phone</span>
                          <p className="font-medium">{watchedValues.owner_phone}</p>
                        </div>
                      </div>
                    )}
                    <div>
